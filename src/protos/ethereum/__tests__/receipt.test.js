...
describe("# marshallTransaction ", () => {
    ...
    test("set object", () => {
        ...
        marshallReceipt(trace, testMsg)
        const receipt = trace.getReceipt().toObject()
        const expected = {
    ...
    test("marshall multiple times", () => {
        ...
        marshallReceipt(trace, testMsg)
        let receipt = trace.getReceipt().toObject()
        ...
        marshallReceipt(trace, testMsg2)
        receipt = trace.getReceipt().toObject()
        expect(receipt.txHash).toEqual(testMsg.txHash)
